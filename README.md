...
1. 在 `src/lib/theme.ts` 注册渲染器类型和能力契约，逐项声明颜色、字体、字号、行高、留白、圆角与标题造型是完整支持、部分支持还是固定。
...
5. 运行 `npm run verify:themes`。校验器会确认每个契约字段都有测试样例，并验证完整或部分支持的配置确实影响输出、固定配置不会泄漏到输出。
