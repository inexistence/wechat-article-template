...
export const DEFAULT_MARKDOWN = `# 橘鸦 AI 早报 2025-09-04
...
> OpenAI 宣布 \`ChatGPT Projects\` 功能全面开放给免费用户，并提升了各层级的文件上传额度，增加项目级记忆控制等新功能。

OpenAI 宣布 \`ChatGPT Projects\` 功能现已向所有 **Free** 用户开放。

此次更新还带来了文件上传额度的提升，并引入新的自定义选项和独立的记忆控制开关，用户现在可以单独决定项目是否启用长期记忆。
...
> **Microsoft Research Cambridge** 团队成功研发出模拟光计算机，并在证券结算和 MRI 扫描加速两大实际问题中验证了其可行性。

团队历时四年打造的模拟光计算机（\`AOC\`）首次在两项高价值优化任务中展现卓越性能，为超越传统 **GPU** 的能效与速度提供了新的可能。

该设备由常用的 \`micro-LED\`、智能手机镜头与传感器组装而成。当前原型包含 **256** 个可调参数，后续计划继续扩展规模。
